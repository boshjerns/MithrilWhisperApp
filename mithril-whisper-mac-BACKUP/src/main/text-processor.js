...
class TextProcessor {
  ...
  async init() {
    try {
      // Determine a writable models directory (avoid app.asar in production)
      const defaultModelsDir = path.join(__dirname, '../../models');
      let resolvedModelsDir = defaultModelsDir;
      try {
        if (String(defaultModelsDir).includes('app.asar')) {
          resolvedModelsDir = path.join(app.getPath('userData'), 'models');
        }
      } catch (_) {}
      this.modelsDir = resolvedModelsDir;
      try {
        if (!fs.existsSync(this.modelsDir)) {
          fs.mkdirSync(this.modelsDir, { recursive: true });
        }
      } catch (e) {
        // Do not fail init if models dir cannot be created; whisper-local does not depend on this path
        console.warn('Skipping models dir creation:', e.message);
      }
      ...
      if (this.useLocalWhisper) {
        ...
        if (this.store) {
          const modelSize = this.store.get('whisperModel', 'base-q5_1');
  ...
  async transcribe(audioData) {
    ...
    try {
      ...
      console.log('Transcription result: length=', transcript ? transcript.length : 0);
      return transcript;
    } catch (error) {
